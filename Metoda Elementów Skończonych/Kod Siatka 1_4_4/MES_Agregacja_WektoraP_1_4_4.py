...
p_global = np.zeros((NodesNumber,1))
...
for i, element in enumerate(ObiektyElementow):
    ...
    for i_local, id_global in enumerate(Elements[i]):

        p_global[id_global-1] += P_local[i_local]
...
output_filename = 'WektorP_Zagregowany_Wyniki_1_4_4.csv'
...
P_results_df = pd.DataFrame(p_global)
