...
# Iteruj przez każdy element i jego lokalne macierze Hbc i H
for element, elem_nodes in zip(ObiektyElementow, Elements):
    ...
    # Dodaj wartości z lokalnych macierzy do odpowiednich miejsc w globalnej macierzy H
...
output_filename = 'HGlobalneZagregowane.csv'
...
pd.DataFrame(H_globalne).to_csv(output_path, index=False, header=False)
print(f"Zagregowana globalna macierz H została zapisana do pliku: {output_path}")